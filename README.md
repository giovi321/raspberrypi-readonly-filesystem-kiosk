...
* Configure WiFi, ssh and anything else you want using Raspberry Pi Imager
...
Edit the `systemd-random-seed` service by issuing `systemctl edit systemd-random-seed.service` and then add the following lines in the correct part of the file (read the comments)
```
[Service]
...
```
apt install -y xinit openbox chromium
```
